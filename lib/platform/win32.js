...
async function processList() {
  ...
  if (!d) return [];
  const raw = arr(d).map((x) => ({
    pid: intOr(x.pid, 0),
    ppid: intOr(x.ppid, null),
    ageS: intOr(x.ageS, null),
  ...
  // `orphaned` means the same thing to a caller on both platforms — "no living
  // parent is responsible for this" — and it is reached by a different road
  // here, because Windows does not reparent to an init process. The parent id
  // simply goes on pointing at a pid that has exited.
  //
  // The caveat is real and belongs in the open: Windows reuses process ids, so
  // a dead parent's number can be held by an unrelated live process, and this
  // then reads `orphaned: false`. It errs toward silence, which is the right
  // direction for a field whose only job is to put a `kill` on someone's screen.
  const live = new Set(raw.map((x) => x.pid));
  const WINDOWS_DIR = /^[a-z]:\\windows\\/i;
  return raw.map((x) => ({
    ...x,
    // No controlling terminal in the Unix sense. null already means "no
    // terminal" to every caller, so it is not a hole — it is the answer.
    tty: null,
    orphaned: x.ppid != null && x.ppid !== 0 && !live.has(x.ppid),
    // Services and the OS itself live under the Windows directory. This is the
    // coarse net; a port that wants the precise one queries Win32_Service for
    // ProcessId and unions it in, exactly as darwin.js unions `launchctl list`.
    systemManaged: WINDOWS_DIR.test(x.command || ''),
  }));
}

// Who is listening, and on what. Same shape and the same null-versus-empty
// distinction as darwin.js: `null` is "could not find out", `[]` is "nothing
// is listening".
async function listeningPorts() {
  const d = await psJson(
    '@(Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue |'
    + ' Select-Object -Property OwningProcess, LocalPort)',
    { timeout: 20000 });
  if (!d) return null;
  return arr(d)
    .map((x) => ({ pid: intOr(x.OwningProcess, 0), port: intOr(x.LocalPort, 0) }))
    .filter((x) => x.pid > 0 && x.port > 0);
}
...
// ---------------------------------------------------------------------------
// WHERE THE COMMANDS THIS PLATFORM PRINTS ARE MEANT TO BE PASTED.
//
// This is here because of a real failure. Somebody pasted a command from this
// panel into Command Prompt — which is what Windows offers when you search for
// "prompt" — and got:
//
//   'Remove-Item' nao e reconhecido como um comando interno ou externo
//
// The command was correct. The panel had simply never said which shell it was
// written for, and the error does not hint at the answer. A panel whose whole
// contract is "I print the command, you run it" has to say where.
// ---------------------------------------------------------------------------
const COMMAND_SHELL = Object.freeze({
  name: 'PowerShell',
  notThis: 'Command Prompt',
  open: 'Press the Windows key, type PowerShell, and press Enter.',
  openAdmin: 'Press the Windows key, type PowerShell, then right-click it and choose "Run as administrator".',
  // The exact sentence Windows prints in both languages the machines this has
  // run on speak, so the screen can name the symptom rather than describe it.
  wrongShellSymptom: 'is not recognized as an internal or external command / nao e reconhecido como um comando interno ou externo',
});
...
module.exports = {
  ...
  // size, and the guards that make a size trustworthy
  COMMAND_SHELL, volumeUsage, dirSizeKB, dirSizesKB, realSizeKB, fileCount, hardlinkRatio,
