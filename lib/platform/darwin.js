...
// `ps` prints elapsed time in three shapes — `MM:SS`, `HH:MM:SS`, `D-HH:MM:SS`
// — and the day field is the one that matters, because everything the Pressure
// tab judges, it judges on age.
...
// Which pids launchd is holding open. Two very different things reparent to
// PID 1 and the process table draws them identically: a process whose shell
// died, and an agent launchd started that way on purpose. Telling them apart is
// platform knowledge, so it is answered here rather than in a collector that
// would have to learn what launchd is.
//
// Only the CURRENT domain is enumerated, which is why this is not the whole
// answer: 21 copies of `distnoted` run on this machine and exactly ONE of them
// appears here — the other twenty live in per-session domains. DAEMON_DIR below
// carries the rest, and both nets are needed.
async function launchdPids() {
  const set = new Set();
  const r = await run('launchctl', ['list'], { timeout: 6000 });
  if (!r.ok) return set;
...
// Where macOS keeps daemons. /usr/bin is deliberately absent: `yes`, `sleep`,
// `dd` and `cat` live there, and those are exactly what a leaked load generator
// is built out of.
const DAEMON_DIR = /^\/(usr\/(libexec|sbin)|System|Library\/(Apple|PrivilegedHelperTools|SystemExtensions))\//;

// The third net, and the one that is easiest to miss. Every helper inside an
// .app bundle reports PID 1 as its parent, because LaunchServices started it
// that way — `Orca Helper` reads exactly like a process whose shell died. A
// GUI app is never an orphaned shell job, on any Mac, so the bundle path is
// proof of who is responsible for it.
...
//
// `comm` stays last because it is the only variable-width column and the regex
// takes the rest of the line. That is also why there is no argv here: `command`
// would have to be last too, and one of the two would be unparseable.
async function processList() {
  const [r, managed] = await Promise.all([
    sh('ps -Axo rss,pid,ppid,%cpu,etime,tty,comm | tail -n +2', { timeout: 15000 }),
    launchdPids(),
  ]);
  if (!r.ok) return [];
  const list = [];
  for (const l of r.out.split('\n')) {
    const mt = l.match(/^\s*(\d+)\s+(\d+)\s+(\d+)\s+([\d.]+)\s+(\S+)\s+(\S+)\s+(.*)$/);
    if (!mt) continue;
    const pid = +mt[2];
    const ppid = +mt[3];
    const command = mt[7];
    list.push({
      rssKB: +mt[1], pid, ppid, cpuPct: +mt[4],
      ageS: elapsedSeconds(mt[5]),
      // `ps` writes `??` for a process with no controlling terminal. null reads
      // as "no terminal" everywhere in this codebase; the literal `??` would
      // read as a terminal named `??`.
      tty: mt[6] === '??' ? null : mt[6],
      command, family: family(command),
      orphaned: ppid === 1,
      systemManaged: managed.has(pid) || DAEMON_DIR.test(command) || APP_BUNDLE.test(command),
    });
  }
  return list;
}

// Who is listening, and on what.
//
// Unprivileged `lsof` sees only this user's own sockets, and that is the right
// scope rather than a limitation: the dev server somebody forgot is theirs.
//
// `null` and `[]` are different answers and both are real. `null` is "I could
// not find out", `[]` is "nothing is listening" — and a caller that conflates
// them tells somebody their orphaned server is harmless.
async function listeningPorts() {
  const r = await sh('lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null', { timeout: 8000 });
  if (!r.ok && !r.out) return null;
  const out = [];
  for (const l of (r.out || '').split('\n')) {
    const m = /^\S+\s+(\d+)\s+.*?:(\d+)\s+\(LISTEN\)/.exec(l);
    if (m) out.push({ pid: +m[1], port: +m[2] });
  }
  return out;
}
...
// ---------------------------------------------------------------------------
// Where the commands this platform prints are meant to be pasted. macOS has one
// obvious answer, and it is still stated rather than assumed — see win32.js for
// what assuming it cost.
const COMMAND_SHELL = Object.freeze({
  name: 'Terminal',
  notThis: null,
  open: 'Press Command and Space, type Terminal, and press Enter.',
  openAdmin: null,
  wrongShellSymptom: null,
});
...
function browserTargets() {
  ...
  for (const b of BROWSERS) {
    // One row per browser. The folders below are still measured and still named
    // inside the row; what collapses is the list of them.
    const group = {
      group: `browser-${b.id}`, groupLabel: `${b.label} caches`,
      groupLose: 'Nothing you typed or saved. Pages you have visited load from the network once more instead of from disk, so the first visit to each is slower, and sites that work offline download their data again. You stay signed in.',
    };

    for (const c of b.caches) {
      // NOTE: this is the whole ~/Library/Caches/<vendor> tree, which on macOS
      // already contains Default/Cache and Default/Code Cache for every
      // profile. It is NOT the Windows shape, where each of those is a separate
      // folder under User Data and had to be listed one by one. Measured before
      // assuming otherwise.
      out.push({
        ...group,
    ...
    // What ~/Library/Caches does NOT cover: these live under Application
    // Support instead. Small on a light machine, several hundred megabytes on
    // one that renders a lot, and they were not being looked at at all.
    const support = (...rest) => p('Library', 'Application Support', b.support, ...rest);
    ...
    for (const prof of ['Default', 'Profile 1', 'Profile 2']) {
      const suffix = prof === 'Default' ? '' : `-${prof.toLowerCase().replace(/\s+/g, '')}`;
      const who = prof === 'Default' ? b.label : `${b.label} (${prof})`;
      out.push({
        ...group,
        id: `browser-sw-${b.id}${suffix}`, path: support(prof, 'Service Worker'),
        ...
        command: `rm -rf ${JSON.stringify(support(prof, 'Service Worker'))}`,
      });
      out.push({
        ...group,
        id: `browser-gpu-${b.id}${suffix}`, path: support(prof, 'GPUCache'),
        label: `${who} graphics cache`, verdict: 'disposable', everyday: true,
        lose: 'Nothing. Rebuilt on demand.',
        command: `rm -rf ${JSON.stringify(support(prof, 'GPUCache'))}`,
      });
    }
    for (const shader of ['ShaderCache', 'GrShaderCache']) {
      out.push({
        ...group,
        id: `browser-shader-${b.id}-${shader.toLowerCase()}`, path: support(shader),
        ...
        command: `rm -rf ${JSON.stringify(support(shader))}`,
      });
    }
  }
...
module.exports = { COMMAND_SHELL,
  ...net,
  // memory and processes
  memoryStats, swapStats, processList, selfProcess, listeningPorts,
  // filesystem primitives
