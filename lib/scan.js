...
async function light() {
  ...
  return { type: 'light', platform: platform.id, memory: mem, volume: vol, ms: Date.now() - t0, at: Date.now() };
}
...
async function deep(log = () => {}) {
  ...
  self.markScan(ms);
  const data = { type: 'deep', platform: platform.id, memory: mem, volume: vol, targets,
