...
async function commitsOffMain(repo) {
  ...
  if (!base) {
    const only = await git(repo, ['rev-list', '--count', '--all', '--not', '--remotes'], 25000);
    return { base: null, branch, off: null, pushed: null, behind: null, remotes: [],
      onlyHere: only.ok ? (parseInt(only.out, 10) || 0) : null };
  }
  ...
  // HOW MUCH OF THIS WORK EXISTS NOWHERE ELSE.
  //
  // `--all --not --remotes` counts every commit reachable from any local ref
  // that is reachable from no remote ref. One command, and it is correct in
  // both of the cases that matter: a repository with no remote at all (every
  // commit it has), and one with a remote that is simply behind (only what has
  // not been pushed). `pushed` above answers whether HEAD is safe; this answers
  // how much is not, which is the number that makes the finding actionable.
  const only = await git(repo, ['rev-list', '--count', '--all', '--not', '--remotes'], 25000);
  ...
  return {
    ...
    // null, not 0, when git could not answer: "no commits are at risk" is a
    // claim, and it is the one claim here that must never be made up.
    onlyHere: only.ok ? (parseInt(only.out, 10) || 0) : null,
