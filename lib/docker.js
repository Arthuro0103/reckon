...
const { run, sh, realKB } = require('./sh');

const RAW = path.join(os.homedir(), 'Library/Containers/com.docker.docker/Data/vms/0/data/Docker.raw');
...
async function volumes() {
  const r = await sh("docker volume ls --format '{{.Name}}' 2>/dev/null", { timeout: 20000 });
  if (!r.ok) return [];
  const names = r.out.split('\n').filter(Boolean);
  const dang = await sh('docker volume ls -qf dangling=true 2>/dev/null', { timeout: 20000 });
  const orphans = new Set((dang.ok ? dang.out : '').split('\n').filter(Boolean));

  const usage = new Map();
  const ps = await sh("docker ps -a --format '{{.ID}}\t{{.Names}}\t{{.State}}' 2>/dev/null", { timeout: 20000 });
...
module.exports = { collect, inVM, toKB, RAW };
