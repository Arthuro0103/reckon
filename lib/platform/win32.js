...
function everydayTargets() {
  ...
  // Chromium-family browsers all use the same layout under a different vendor
  // folder, so one shape covers six browsers.
  const CHROMIUM = [
    { id: 'chrome', label: 'Chrome', dir: ['Google', 'Chrome'] },
    { id: 'edge', label: 'Edge', dir: ['Microsoft', 'Edge'] },
    { id: 'brave', label: 'Brave', dir: ['BraveSoftware', 'Brave-Browser'] },
    { id: 'vivaldi', label: 'Vivaldi', dir: ['Vivaldi'] },
    { id: 'opera', label: 'Opera', dir: ['Programs', 'Opera'] },
    { id: 'operagx', label: 'Opera GX', dir: ['Programs', 'Opera GX'] },
  ];

  const out = [];
  for (const b of CHROMIUM) {
    const base = path.join(local, ...b.dir, 'User Data', 'Default');
    out.push({
      id: `browser-cache-${b.id}`, path: path.join(base, 'Cache'),
      ...
      command: `Remove-Item -LiteralPath "${path.join(base, 'Cache')}" -Recurse -Force`,
    });
    out.push({
      id: `browser-sw-${b.id}`, path: path.join(base, 'Service Worker'),
      ...
      command: `Remove-Item -LiteralPath "${path.join(base, 'Service Worker')}" -Recurse -Force`,
    });
  }

  out.push(
    {
      id: 'firefox-cache', path: path.join(local, 'Mozilla', 'Firefox', 'Profiles'),
    ...
    },
    {
      ...
      id: 'windows-old', path: path.join(sysDrive + '\\', 'Windows.old'),
    ...
    {
      id: 'windows-temp', path: path.join(local, 'Temp'),
      ...
      command: `Get-ChildItem -LiteralPath "${path.join(local, 'Temp')}" -Force | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue`,
    },
    {
      id: 'delivery-optimization', path: path.join(win, 'SoftwareDistribution', 'Download'),
    ...
    {
      id: 'recycle-bin', path: path.join(sysDrive + '\\', '$Recycle.Bin'),
      label: 'Recycle Bin', verdict: 'disposable', everyday: true,
      lose: 'Whatever you already put in it.',
      command: 'Clear-RecycleBin -Force',
    },
    {
      id: 'thumbnail-cache', path: path.join(local, 'Microsoft', 'Windows', 'Explorer'),
