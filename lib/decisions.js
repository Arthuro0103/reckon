...
function classify(id) {
  ...
  if (/browser-cache-|firefox-cache/.test(id)) return 'browsing';
  if (/browser-sw-/.test(id)) return 'browsing';
  if (/trash|recycle-bin|windows-old|windows-temp|delivery-optimization/.test(id)) return 'bin';
  if (/mail-downloads|photo-analysis|quicklook|thumbnail-cache|crashreports|ios-backups/.test(id)) return 'system';
  // Developer.
  ...
  if (/npm-|hf-|pip|brew|uv|pnpm|cargo/.test(id)) return 'cache';
  return 'other';
}
